...
commandFiles.forEach(file => {
    if (file.endsWith('.js')) {
        ...
        // const commandPath = file; // file is already full path
        try {
...
async function handleMessage(sock, msg) {
    try {
        ...
        // 2. Auto-Read Logic (Moved to execute before commands)
        ...
        if (!messageText.startsWith(settings.prefix)) {
            ...
            if (cleanMsg === '1' || cleanMsg === '2' || cleanMsg === '3') {
                ...
                const { setUserLanguage, getUser } = require('./userLogger');
                setUserLanguage(senderId, selectedLang);

                // Add to userLogger if not exists
                try {
        ...
        if ((!userProfile || !userProfile.language) && !isGroup && !msg.key.fromMe) {
            ...
            if (commandName !== 'setlang' && commandName !== 'لغة') {
                ...
                // Do not return here, simply prompt and let the command continue
            }
        }
        ...
        if (!commands.has(commandName)) {
            ...
            if (actualCommandName && commands.has(actualCommandName)) {
                ...
                if (typeof command === 'function' || (command && typeof command.execute === 'function')) {
                    ...
                    // 2. Auto-Read Logic (Moved here to only read when command is triggered)
                    try {
                        const configPath = path.join(__dirname, '../data/config.json');
                        if (fs.existsSync(configPath)) {
                            const config = JSON.parse(fs.readFileSync(configPath));
                            if (config.AUTOREAD === "true" || config.AUTOREAD === true) {
