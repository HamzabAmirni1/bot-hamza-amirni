...
const { getUptime, readAntiCallState, writeAntiCallState } = require('../commands/lib/utils');
const { logError } = require('./supabase');
...
const aliasMap = {
    ...
    'مجموعة': 'groupinfo', 'group': 'groupinfo',
    ...
    'pinterest': 'pinterest', 'بنترست': 'pinterest', 'pint': 'pinterest', 'pinterestdl': 'pinterestdl',
    ...
    'wc': 'worldcup', 'worldcup': 'worldcup', 'كأس-العالم': 'worldcup',
    ...
    'maroc-flag': 'maroc-flag', 'علم-المغرب': 'maroc-flag',

    // Dev inbox aliases
    'msgtoown': 'msgtodev', 'راسل-المالك': 'msgtodev', 'راسلني': 'msgtodev', 'contactdev': 'msgtodev',
    'تليشارجي': 'apk'
};
...
// --- HELPERS CREATOR FOR NATIVE COMMANDS ---
function makeHelpers(commandName) {
    const helpers = {
        commandUsage: global._cmdStats || {},
        commandErrors: global._cmdErrors || {},
        getUptime,
        readAntiCallState,
        writeAntiCallState,
        isTelegram: false,
        isFacebook: false,
        command: commandName,
        // Delegate Map methods to the original commands Map
        get: (key) => commands.get(key),
        has: (key) => commands.has(key),
        set: (key, val) => commands.set(key, val),
        delete: (key) => commands.delete(key),
        forEach: (cb) => commands.forEach(cb),
        keys: () => commands.keys(),
        values: () => commands.values(),
        entries: () => commands.entries(),
        size: commands.size
    };
    helpers.commandsMap = commands;
    return helpers;
}
...
// Helper: check if autoread is enabled (reads root-level data/config.json)
...
async function handleMessage(sock, msg) {
    try {
        ...
        // 🔵 AUTO-READ (VU) SHOW BLUE TICKS FOR ALL INCOMING MESSAGES
        if (isAutoReadEnabled() && msg.key) {
        ...
        // 🚀 NON-BLOCKING LANGUAGE SELECTION & PROMPT 🚀
        const { getUser, setUserLanguage } = require('./userLogger');
        let userProfile = getUser(senderId);
        const cleanMsg = messageText.trim();
        
        // 1. Intercept explicit numeric language selections in private chat for new users
        const isLangChoice = !isGroup && ['1', '2', '3'].includes(cleanMsg) && (!userProfile || !userProfile.language);
        ...
        if (!msg.key.fromMe && isLangChoice) {
            ...
            return; // Handled language selection, stop execution
        }

        // 2. Track if user is new (no profile or no language set) to trigger welcome menu asynchronously
        let isNewUser = false;
        if (!userProfile) {
            addUser({ id: senderId, name: msg.pushName || '' });
            userProfile = getUser(senderId);
            isNewUser = true;
        } else if (!userProfile.language) {
            isNewUser = true;
        }

        // 3. Asynchronously send language prompt if new user in private chat (without blocking command)
        if (isNewUser && !isGroup && !msg.key.fromMe && !isSetlangCommand) {
            (async () => {
                try {
                    ...
                    // Cache generated imageMessage to prevent redundant uploads/lag
                    async function createHeaderImage(imagePath) {
                        global.welcomeImageCache = global.welcomeImageCache || {};
                        if (global.welcomeImageCache[imagePath]) {
                            return global.welcomeImageCache[imagePath];
                        }
                        try {
                            const { imageMessage } = await generateWAMessageContent({ image: fs.readFileSync(imagePath) }, { upload: sock.waUploadToServer });
                            global.welcomeImageCache[imagePath] = imageMessage;
                            return imageMessage;
                        } catch (e) {
                            console.error(`Failed to load welcome card image: ${imagePath}. Error: ${e.message}`);
                            const fallbackPath = path.join(process.cwd(), 'media/hamza.jpg');
                            if (global.welcomeImageCache[fallbackPath]) {
                                return global.welcomeImageCache[fallbackPath];
                            }
                            try {
                                const { imageMessage } = await generateWAMessageContent({ image: fs.readFileSync(fallbackPath) }, { upload: sock.waUploadToServer });
                                global.welcomeImageCache[fallbackPath] = imageMessage;
                                return imageMessage;
                            } catch (err) {
                                return null;
                            }
                    ...
                    const langCardsData = [
                        {
                            ...
                            buttonText: "العربية 🇸🇦",
                            id: ".setlang ar",
                            img: path.join(process.cwd(), 'media/menu/lang_ar.png')
                        },
                        {
                            ...
                            buttonText: "English 🇺🇸",
                            id: ".setlang en",
                            img: path.join(process.cwd(), 'media/menu/lang_en.png')
                        },
                        {
                            ...
                            buttonText: "الدارجة 🇲🇦",
                            id: ".setlang ma",
                            img: path.join(process.cwd(), 'media/menu/lang_ma.png')
                        }
                    ];
        ...
        if (!messageText.startsWith(settings.prefix)) {
            ...
            if (links && links.length > 0) {
                ...
                if (/(facebook\.com|fb\.watch|fb\.com)/i.test(cleanText)) {
                    ...
                    autoCommand = 'twitter';
                } else if (/(?:pin\.it\/|pinterest\.(?:com|fr|de|es|it|co\.uk)\/pin\/)/i.test(cleanText)) {
        ...
        if (!commands.has(commandName)) {
            ...
            if (actualCommandName && commands.has(actualCommandName)) {
                ...
                if (typeof command === 'function' || (command && typeof command.execute === 'function')) {
                    ...
                    // Track alias command stats
                    try {
                        if (!global._cmdStats) global._cmdStats = {};
                        if (!global._cmdStats[actualCommandName]) global._cmdStats[actualCommandName] = 0;
                        global._cmdStats[actualCommandName]++;
                        if (!global._activeUsers) global._activeUsers = new Set();
                        global._activeUsers.add(senderId);
                        if (!global._activityLog) global._activityLog = [];
                        global._activityLog.unshift({ time: Date.now(), cmd: actualCommandName, user: senderId, chat: chatId });
                        if (global._activityLog.length > 100) global._activityLog.length = 100;
                    } catch(_) {}
                    const helpers = makeHelpers(actualCommandName);
                    try {
                        if (typeof command === 'function') {
                            console.log(`[Handler] Executing Aliased Function: ${actualCommandName}`);
                            await command(sock, chatId, msg, args, helpers, userLang, match);
                        } else {
                            console.log(`[Handler] Executing Aliased Command Object: ${actualCommandName}`);
                            await command.execute(sock, chatId, msg, args, helpers, userLang, match);
                        }
                    } catch (cmdErr) {
                        console.error(`[Handler] Error executing aliased command ${actualCommandName}:`, cmdErr);
                        try {
                            if (!global._cmdErrors) global._cmdErrors = {};
                            if (!global._cmdErrors[actualCommandName]) global._cmdErrors[actualCommandName] = 0;
                            global._cmdErrors[actualCommandName]++;
                        } catch (_) {}
                        try {
                            await logError(actualCommandName, cmdErr.message || String(cmdErr), 'WA');
                        } catch (_) {}
                        try {
...
⚔️ ${settings.botName}`, msg);
        ...
        if (command) {
            ...
            try {
                if (!global._cmdStats) global._cmdStats = {};
                if (!global._cmdStats[commandName]) global._cmdStats[commandName] = 0;
                global._cmdStats[commandName]++;
                if (!global._activeUsers) global._activeUsers = new Set();
                global._activeUsers.add(senderId);
                if (!global._activityLog) global._activityLog = [];
                global._activityLog.unshift({ time: Date.now(), cmd: commandName, user: senderId, chat: chatId });
                if (global._activityLog.length > 100) global._activityLog.length = 100;
            } catch(_) {}
            ...
            const helpers = makeHelpers(commandName);
            try {
                if (typeof command === 'function') {
                    await command(sock, chatId, msg, args, helpers, userLang, match);
                ...
                console.error(`[Handler] Error executing command ${commandName}:`, cmdErr);
                try {
                    if (!global._cmdErrors) global._cmdErrors = {};
                    if (!global._cmdErrors[commandName]) global._cmdErrors[commandName] = 0;
                    global._cmdErrors[commandName]++;
                } catch (_) {}
                try {
                    await logError(commandName, cmdErr.message || String(cmdErr), 'WA');
                } catch (_) {}
                try {
                    await sock.sendMessage(chatId, { text: t('common.error', userLang) }, { quoted: msg });
                } catch (_) {}
