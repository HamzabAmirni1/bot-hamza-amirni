...
const { writeExif } = require('./myfunc.js');
let conv = require('./sticker.js')
...
exports.makeWASocket = function makeWASocket(connectionOptions, options = {}) {
    /**
...
exports.smsg = function smsg(conn, m, hasParent) {
    if (!m) return m
    /**
     * @type {import('@whiskeysockets/baileys').proto.WebMessageInfo}
     */
...
exports.logic = function logic(check, inp, out) {
    if (inp.length !== out.length) throw new Error('Input and Output must have same length')
    for (let i in inp) if (util.isDeepStrictEqual(check, inp[i])) return out[i]
    return null
}
exports.protoType = function protoType() {
...
async function generateProfilePicture(mediaUpload) {
    ...
    const { read, MIME_JPEG, AUTO } = await Promise.resolve().then(async () => require('jimp'))
