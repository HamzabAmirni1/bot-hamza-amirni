...
// ─── PROVIDER 0: @distube/ytdl-core (local, most reliable) ───────────────────
async function getDistube(url, isAudio) {
    const ytdl = require('@distube/ytdl-core');
    const info = await ytdl.getInfo(url, { requestOptions: { headers: { 'User-Agent': COMMON_HEADERS['User-Agent'] } } });
    const title = info.videoDetails.title;
    let format;
    if (isAudio) {
        format = ytdl.chooseFormat(info.formats, { quality: 'highestaudio', filter: 'audioonly' });
    } else {
        // prefer mp4 with both video+audio (progressive), else best quality
        const progressive = info.formats.filter(f => f.hasVideo && f.hasAudio && f.container === 'mp4');
        if (progressive.length > 0) {
            progressive.sort((a, b) => (parseInt(b.qualityLabel) || 0) - (parseInt(a.qualityLabel) || 0));
...
// ─── PROVIDER 1: ytconvert.org (powers ytmp3.gg) ─────────────────────────────
...
// ─── PROVIDER 2: savetube.vip (AES decrypt, stable CDN) ─────────────────────
...
// ─── PROVIDER 3: y2down / p.savenow.to ───────────────────────────────────────
...
async function getOotaizumi(url, isAudio) {
    ...
    const res = await axiosIgnoreSSL.get(endpoint, { timeout: 20000 });
    const d = res.data;
...
// ─── PROVIDER 6: snap-any API (new, often works) ─────────────────────────────
async function getSnapAny(url, isAudio) {
    const res = await axios.post('https://snap-any.com/api/download', {
        url,
        format: isAudio ? 'mp3' : 'mp4'
    }, {
    ...
    const dl = res.data?.url || res.data?.download_url || res.data?.data?.url;
    if (dl) return { download: dl, title: res.data?.title || 'YouTube' };
    throw new Error('SnapAny: no download URL');
}

// ─── PROVIDER 7: dlpanda / yt-dlp public mirrors ─────────────────────────────
async function getDlPanda(url, isAudio) {
    const res = await axios.get(`https://api.dlpanda.com/v1/youtube`, {
        params: { url, format: isAudio ? 'mp3' : 'mp4' },
        headers: COMMON_HEADERS,
        timeout: 20000
    });
    const dl = res.data?.url || res.data?.download_url;
    if (dl) return { download: dl, title: res.data?.title || 'YouTube' };
    throw new Error('DlPanda: no download URL');
}

// ─── MAIN ENTRY POINT ─────────────────────────────────────────────────────────
/**
 * Downloads YouTube audio or video using multiple reliable providers in fallback order.
 * Provider 0 (distube/ytdl-core) is the local library — most reliable, no external API.
 * @param {string} url - YouTube URL
 * @param {'mp3'|'mp4'|'audio'|'video'} type
 * @returns {Promise<{download:string, downloadUrl:string, title:string, thumb?:string, referer?:string}|null>}
 */
async function downloadYouTube(url, type = 'mp3') {
    const isAudio = type === 'mp3' || type === 'audio';

    const providers = [
        { name: 'Distube (local)', fn: () => getDistube(url, isAudio) },
        ...
        { name: 'SnapAny', fn: () => getSnapAny(url, isAudio) },
        { name: 'DlPanda', fn: () => getDlPanda(url, isAudio) },
    ];
    ...
    for (const provider of providers) {
        try {
            ...
            if (result?.download) {
                // Verify the download URL is actually a media file and not an HTML error page
                const testBuf = await getBuffer(result.download, result.referer);
                if (testBuf && testBuf.length > 0) {
                    const sample = testBuf.slice(0, 500).toString('utf8').toLowerCase();
                    const isHtml = sample.includes('<html') || sample.includes('<!doctype') || sample.includes('<div') || sample.includes('<script');
                    if (isHtml) {
                        console.log(`[YTDL] ❌ ${provider.name} returned an HTML error/challenge page instead of media. Trying next.`);
                        continue;
                    }
                    console.log(`[YTDL] ✅ Success via ${provider.name} (verified media buffer size: ${testBuf.length} bytes)`);
                    result.downloadUrl = result.download;
                    result.buffer = testBuf; // Cache buffer to avoid redundant HTTP requests in callers
                    return result;
                } else {
                    console.log(`[YTDL] ❌ ${provider.name} returned an empty download stream.`);
                }
            }
        } catch (e) {
