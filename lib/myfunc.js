...
function smsg(conn, m, store) {
    ...
    if (m.message) {
        ...
        const searchForCommand = (obj, depth = 0) => {
            ...
            // Priority Keys
            const pKeys = ['selectedId', 'selectedButtonId', 'selectedRowId', 'id'];
            for (const k of pKeys) {
                if (typeof obj[k] === 'string' && (obj[k].startsWith('.') || obj[k].startsWith('!') || obj[k].startsWith('/'))) {
                    extractedId = obj[k];
                    return;
                }
            }

            // Native Flow JSON Parsing
            if (obj.paramsJson && typeof obj.paramsJson === 'string') {
                try {
                    const parsed = JSON.parse(obj.paramsJson);
                    if (parsed.id && (parsed.id.startsWith('.') || parsed.id.startsWith('!') || parsed.id.startsWith('/'))) {
