...
function smsg(conn, m, store) {
    ...
    if (m.message) {
        ...
        // 1. Correctly unwrap ViewOnce layers
        ...
        // 2. Priority extraction: ID for buttons/interactive messages
        let extractedId = '';
        if (m.mtype === 'buttonsResponseMessage') extractedId = m.msg.selectedButtonId;
        else if (m.mtype === 'listResponseMessage') extractedId = m.msg.singleSelectReply.selectedRowId;
        else if (m.mtype === 'templateButtonReplyMessage') extractedId = m.msg.selectedId;
        else if (m.mtype === 'interactiveResponseMessage') {
            try {
                const paramsJson = m.msg.nativeFlowResponseMessage?.paramsJson;
                if (paramsJson) extractedId = JSON.parse(paramsJson).id;
            } catch (e) { }
        }

        // 3. Fallback to text properties
        ...
        // 4. Set m.text to match m.body for uniform processing
        m.text = m.body;
