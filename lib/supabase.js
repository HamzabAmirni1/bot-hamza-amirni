...
// Set global WebSocket to support Node.js versions without native WebSocket globally
...
const db = {
    ...
    async clearAllErrors() {
        if (!supabase) return false;
        try {
            const { error } = await supabase.from('error_logs').delete().neq('command', '');
            if (error) throw error;
            return true;
        } catch (e) {
            console.error('[Supabase] clearAllErrors Error:', e.message);
            return false;
        }
    },
