...
async function stickerCommand(sock, chatId, message, args, commands, userLang) {
    ...
    if (!mediaMessage) {
        const usageText = userLang === 'ma'
            ? "⚠️ *كيفاش تخدمو:*\nصيفط تصويرة ولا فيديو قصير (قل من 10 ثواني) وكتب *.sticker* فالعنوان، ولا جاوب عليه."
            : userLang === 'ar'
                ? "⚠️ *طريقة الاستخدام:*\nأرسل صورة أو فيديو قصير واكتب *.sticker*، أو قم بالرد على الوسائط."
                : "⚠️ *Usage:*\nSend an image/video and caption it with *.sticker*, or reply to it.";
    ...
    try {
        ...
        if (!mediaBuffer) {
            ...
            await sock.sendMessage(chatId, { text: errText });
            return;
        }
        ...
        const json = {
            ...
            'sticker-pack-name': settings.packname || 'Hamza Bot',
            'emojis': ['🤖']
        };
        ...
        const errText = userLang === 'ma'
            ? "❌ *الستيكر عكّس. عاود جرب.*"
            : userLang === 'ar'
                ? "❌ *حدث خطأ أثناء صنع الملصق.*"
